...
This guide is based on the main [`gitlab-org/khulnasoft-vscode-extension` security release process](https://gitlab.com/gitlab-org/khulnasoft-vscode-extension/-/blob/main/docs/developer/security-releases.md)

## DO NOT PUSH TO `khulnasoft/web-ide`
...
To that end, you'll need to be sure that security vulnerabilities are fixed in the [Security Repo](https://gitlab.com/gitlab-org/security/khulnasoft-web-ide).

This is fundamental to our security release process because the [Security Repo](https://gitlab.com/gitlab-org/security/khulnasoft-web-ide) is not publicly-accessible.

## Process

A security fix starts with an issue identifying the vulnerability. This may be either in the main `gitlab-org/gitlab` project or the `khulnasoft/web-ide` project.

Once a security issue is assigned to a developer, we follow the same merge request and code review process as any other change, but on the [Security Repo](https://gitlab.com/gitlab-org/security/khulnasoft-web-ide).
...
1. Start a new pipeline on the relevant `security-release-` branch with the following CI/CD job variables:
   - **KHULNASOFT_WEB_IDE_VERSION:** The patch release version, for example `0.0.1-dev-20230524134151-patch-1`
...
1. [Create an MR](https://gitlab.com/gitlab-org/security/khulnasoft-web-ide/-/merge_requests/new?merge_request%5Bsource_project_id%5D=47566511&merge_request%5Bsource_branch%5D=&merge_request%5Btarget_project_id%5D=35104827&merge_request%5Btarget_branch%5D=main) from the security project:
   ...
   - **Target project:** `khulnasoft/web-ide`
   - **Target branch:** `main`
...
The [Request CVE Number](https://gitlab.com/gitlab-org/khulnasoft-vscode-extension/-/blob/main/docs/developer/security-releases.md#request-cve-number) section
