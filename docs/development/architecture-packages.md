...
flowchart TD
  ...
  vscode-buildA --> khulnasoft-web-ide-vscode-fork(https://...khulnasoft-web-ide-vscode-fork...tar.gz)
  ...
  subgraph vscode-extension-khulnasoft-vscode-extension
      vscode-extension-khulnasoft-vscode-extensionA[dist/khulnasoft-vscode-extension/dist-browser]
      vscode-extension-khulnasoft-vscode-extensionB[gitlab_vscode_extension_version.json]
  end
  vscode-extension-khulnasoft-vscode-extensionA --> ExtArtifact[Release Artifact]
  ExtArtifact --> khulnasoft-vscode-extension_repo[gitlab-org/khulnasoft-vscode-extension releases]
  ...
  subgraph web-ide [web-ide]
      ...
      khulnasoft-vscode-extension[khulnasoft-vscode-extension]

      subgraph web-ide-public[dist/public]
          web-ide-publicA[main.js]
          web-ide-publicB[vscode/...]
          web-ide-publicC[vscode/extensions/khulnasoft-web-ide]
          web-ide-publicD[vscode/extensions/khulnasoft-vscode-extension]
      end
  end
  web-ide-publicA --> vscode-bootstrap
  web-ide-publicB --> vscode-build
  web-ide-publicC --> vscode-extension-web-ide
  web-ide-publicD --> vscode-extension-khulnasoft-vscode-extension
```
...
The main artifact of the `web-ide` repository is the `@khulnasoft/web-ide` npm package,
...
- `dist/index.js`, which is the main export of the package and provides methods
  for initializing the KhulnaSoft Web IDE. It's expected that this module is simply
...
- `dist/public/...`, which contains assets that should be self-hosted by the client.
  ...
  (see [the main KhulnaSoft project for an example](https://gitlab.com/gitlab-org/gitlab/-/blob/a35dcacbc43060de1e08c7e559d6cc864e68570b/config/webpack.config.js#L663)).
  ...
  - `vscode/...`, which is the VS Code browser build. This comes from the
    ...
    which uses an artifact from the [`khulnasoft-web-ide-vscode-fork`](https://gitlab.com/khulnasoft/web-ide-vscode-fork) project.
  - `vscode/extensions/khulnasoft-web-ide/...`, which is the main VS Code
    extension that powers the KhulnaSoft Web IDE environment. These assets
  ...
  - `main.js`, which is the entry point that reads the configuration
    provided by the `@khulnasoft/web-ide` client, loads the necessary
...
**Should `@khulnasoft/web-ide` have any dependencies?**

No.

Any dependencies here would become transitive dependencies of `@khulnasoft/web-ide` clients.
...
when clients try to install the `@khulnasoft/web-ide` package from npm.
...
in the main `@khulnasoft/web-ide` package.
...
- `dist/main.js`, which is copied to the main `@khulnasoft/web-ide` package.
  `@khulnasoft/web-ide` automatically loads this script into the
  iframe it creates for the KhulnaSoft Web IDE instance.
...
The KhulnaSoft Web IDE has 3 runtime environments:

- `Main`: the main runtime environment that imports and runs the
  `@khulnasoft/web-ide` package.
- `Iframe`: the iframe environment created by `@khulnasoft/web-ide`,
...
creates the KhulnaSoft Web IDE experience.
...
The `vscode-extension-khulnasoft-vscode-theme` package contains the source for the GitLab
...
The `vscode-extension-khulnasoft-vscode-extension` internal package is used to provide a browser build of the KhulnaSoft VS Code extension.

The browser build itself is **not** checked into source control. Instead,
the assets are downloaded based on the location specified in
[`gitlab_vscode_extension_version.json`](../../packages/vscode-extension-khulnasoft-vscode-extension/gitlab_vscode_extension_version.json).

The main artifact of this package is:

- `dist/khulnasoft-vscode-extension/dist-browser...`, which contains the extracted KhulnaSoft VS Code extension assets specified
...
- `extensions.getExtension('gitlab.khulnasoft-web-ide')` which returns the `WebIDEExtension` type.
...
with communicating to a KhulnaSoft instance through its REST API.
...
- [`khulnasoft-web-ide` architecture](https://youtu.be/-hnpY4UObZg)
